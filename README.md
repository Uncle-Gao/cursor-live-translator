# Cursor-Live-Translator (V2.5.1)：全场景 AI 实时本地化引擎
...
本项目是专门为 Cursor 定制的高性能实时翻译引擎。通过 **V2.5.1 Translation Bridge (翻译代理桥)** 架构，实现了离线字典匹配、动态变量正则解析与跨窗口 AI 实时翻译的深度融合。

> [!IMPORTANT]
> **架构重大突破：跨窗口通讯穿透与插件全量汉化**
> 在最新的 **V2.5.1 版本** 中，我们解决了长期困扰插件 Webview（如 Claude Code）的 CSP 联网限制问题。通过 `window.top` 直连通讯方案与递归广播机制，主窗口的 AI 能量可以无缝穿透至所有层级的插件内部，实现真正的“全场景汉化”。
...
| **扩展性** | 无。 | 仅限主窗口。 | **全场景插件支持**。支持 Webview 与 Shadow DOM 递归。 |
...
按下 `Cmd + Option + I` (Mac) 或 `F12` (Win) 查看控制台。若看到紫色印记 `[Cursor-Live-Translator] [Workbench] 动力系统就绪 | V2.5.1`，则代表引擎已开启。

## 🛠️ 故障排查与诊断工具 (V2.5.1+)

如果遇到特定区域（如 Agent 侧边栏）无法翻译或 Loading 动画不消失，可以通过控制台 (**Console**) 调用以下内置函数进行排查：

- **`__I18N_DIAGNOSE__()`**：
  - **功能**：打印当前翻译管线的实时诊断包（Pending、In-flight、Error 任务队列）。
- **`__I18N_TRANSLATE__("text")`**：
  - **功能**：手动测试单条翻译连通性，用于锁定是 API 问题还是 DOM 渲染问题。
- **`__I18N_SMOKE_TEST__()`**：
  - **运行**：随引擎启动自动执行。在控制台搜索 `[I18N-SMOKE]` 即可确认当前 API 通道是否畅通。
